...
class FileTransferTest {
    ...
    fun `encode and decode file packet with all fields should preserve data`() {
        ...
        val originalPacket = BluewhaleFilePacket(
            fileName = "test.png",
            mimeType = "image/png",
            fileSize = 1024000,
        ...
        val decoded = BluewhaleFilePacket.decode(encoded!!)
    ...
    fun `encode file packet with filename should include filename TLV`() {
        ...
        val packet = BluewhaleFilePacket(
    ...
    fun `replaceFilePathInContent should correctly format content markers for different file types`() {
        ...
        val imageMessage = BluewhaleMessage(
            id = "test1",
            sender = "alice",
            senderPeerID = "12345678",
            content = "/data/user/0/com.bluewhale.android/files/images/photo.jpg",
            type = BluewhaleMessageType.Image,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val audioMessage = BluewhaleMessage(
            id = "test2",
            sender = "bob",
            senderPeerID = "87654321",
            content = "/data/user/0/com.bluewhale.android/files/audio/voice.amr",
            type = BluewhaleMessageType.Audio,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val fileMessage = BluewhaleMessage(
            id = "test3",
            sender = "charlie",
            senderPeerID = "11223344",
            content = "/data/user/0/com.bluewhale.android/files/documents/document.pdf",
            type = BluewhaleMessageType.File,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )
        ...
        result = result.replace(
            "/data/user/0/com.bluewhale.android/files/images/photo.jpg",
            "[image] photo.jpg"
        )
