...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = com.bluewhale.android.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = com.bluewhale.android.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    
    // MARK: - Public Message Management
    
    fun addMessage(message: BluewhaleMessage) {
        ...
        try { com.bluewhale.android.services.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
    }

    // Log a system message into the main chat (visible to user)
    fun addSystemMessage(text: String) {
        val sys = BluewhaleMessage(
            sender = "system",
            content = text,
            timestamp = Date(),
            isRelay = false
    ...
    fun addChannelMessage(channel: String, message: BluewhaleMessage) {
        ...
        try { com.bluewhale.android.services.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
    ...
    fun addPrivateMessage(peerID: String, message: BluewhaleMessage) {
        ...
        // Reflect into process-wide store
        try { com.bluewhale.android.services.AppStateStore.addPrivateMessage(peerID, message) } catch (_: Exception) { }
    ...
    fun addPrivateMessageNoUnread(peerID: String, message: BluewhaleMessage) {
        val currentPrivateChats = state.getPrivateChatsValue().toMutableMap()
        if (!currentPrivateChats.containsKey(peerID)) {
            currentPrivateChats[peerID] = mutableListOf()
        }
     ...
     */
    fun generateMessageKey(message: BluewhaleMessage): String {
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { com.bluewhale.android.services.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
