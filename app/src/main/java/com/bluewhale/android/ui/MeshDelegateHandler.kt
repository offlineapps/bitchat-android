...
import com.bluewhale.android.ui.NotificationTextUtils
...
class MeshDelegateHandler(
                ...
                if (state.getJoinedChannelsValue().contains(message.channel)) {
                    ...
                    val viewingGeohash = try {
                        if (channel.startsWith("geo:")) {
                            ...
                            selected is com.bluewhale.android.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
                        } else false
    ...
    override fun didUpdatePeerList(peers: List<String>) {
        coroutineScope.launch {
            ...
            runCatching { com.bluewhale.android.services.MessageRouter.tryGetInstance()?.onPeersUpdated(peers) }
                ...
                if (isNostrAlias || isNoiseHex) {
                    ...
                    val canonical = com.bluewhale.android.services.ConversationAliasResolver.resolveCanonicalPeerID(
                        ...
                        nostrPubHexForAlias = { alias ->
                            ...
                            if (com.bluewhale.android.nostr.GeohashAliasRegistry.contains(alias)) {
                                com.bluewhale.android.nostr.GeohashAliasRegistry.get(alias)
                            } else {
                                // Best-effort: derive pub hex from favorites mapping for mesh nostr_ aliases
                                val prefix = alias.removePrefix("nostr_")
                                val favs = try { com.bluewhale.android.favorites.FavoritesPersistenceService.shared.getOurFavorites() } catch (_: Exception) { emptyList() }
                                favs.firstNotNullOfOrNull { rel ->
                                    rel.peerNostrPublicKey?.let { s ->
                                        runCatching { com.bluewhale.android.nostr.Bech32.decode(s) }.getOrNull()?.let { dec ->
                    ...
                    if (canonical != currentPeer) {
                        ...
                        com.bluewhale.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, canonical, listOf(currentPeer))
                    ...
                    val favoriteRel = try {
                        ...
                        if (noiseKey != null) {
                            com.bluewhale.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                        } else null
                    ...
                    if (favoriteRel?.isMutual == true) {
                        ...
                        if (noiseHex != currentPeer) {
                            com.bluewhale.android.services.ConversationAliasResolver.unifyChatsIntoPeer(
                                state = state,
            ...
            peers.forEach { pid ->
                try {
                    ...
                    val npub = com.bluewhale.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKey)
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.bluewhale.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
     ...
     */
    private fun checkAndTriggerMeshMentionNotification(message: BluewhaleMessage) {
        try {
     ...
     */
    private fun sendReadReceiptIfFocused(message: BluewhaleMessage) {
     ...
     */
    fun getPeerInfo(peerID: String): com.bluewhale.android.mesh.PeerInfo? {
