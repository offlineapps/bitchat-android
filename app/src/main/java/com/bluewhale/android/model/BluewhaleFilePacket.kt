...
/**
 * BluewhaleFilePacket: TLV-encoded file transfer payload for BLE mesh.
 * TLVs:
 *  - 0x01: filename (UTF-8)
 ...
 */
data class BluewhaleFilePacket(
    val fileName: String,
    val fileSize: Long,
    val mimeType: String,
    ...
    fun encode(): ByteArray? {
        try {
            android.util.Log.d("BluewhaleFilePacket", "🔄 Encoding: name=$fileName, size=$fileSize, mime=$mimeType")
        ...
        if (nameBytes.size > 0xFFFF || mimeBytes.size > 0xFFFF) {
                android.util.Log.e("BluewhaleFilePacket", "❌ TLV field too large: name=${nameBytes.size}, mime=${mimeBytes.size} (max: 65535)")
                return null
            }
            if (content.size > 0xFFFF) {
                android.util.Log.d("BluewhaleFilePacket", "📦 Content exceeds 65535 bytes (${content.size}); will be split into multiple CONTENT TLVs")
            } else {
                android.util.Log.d("BluewhaleFilePacket", "📏 TLV sizes OK: name=${nameBytes.size}, mime=${mimeBytes.size}, content=${content.size}")
            }
        ...
        val result = buf.array()
            android.util.Log.d("BluewhaleFilePacket", "✅ Encoded successfully: ${result.size} bytes total")
            return result
        } catch (e: Exception) {
            android.util.Log.e("BluewhaleFilePacket", "❌ Encoding failed: ${e.message}", e)
            return null
        }
    }

    companion object {
        fun decode(data: ByteArray): BluewhaleFilePacket? {
            android.util.Log.d("BluewhaleFilePacket", "🔄 Decoding ${data.size} bytes")
            try {
                var off = 0
                ...
                val result = BluewhaleFilePacket(n, s, m, c)
                android.util.Log.d("BluewhaleFilePacket", "✅ Decoded: name=$n, size=$s, mime=$m, content=${c.size} bytes")
                return result
            } catch (e: Exception) {
                android.util.Log.e("BluewhaleFilePacket", "❌ Decoding failed: ${e.message}", e)
                return null
            }
        }
    }
