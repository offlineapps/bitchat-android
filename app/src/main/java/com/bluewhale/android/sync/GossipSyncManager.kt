...
import com.bluewhale.android.mesh.BluetoothPacketBroadcaster
...
class GossipSyncManager(
    ...
    interface Delegate {
        ...
        fun sendPacketToPeer(peerID: String, packet: BluewhalePacket)
        fun signPacketForBroadcast(packet: BluewhalePacket): BluewhalePacket
    }
    ...
    private val messages = LinkedHashMap<String, BluewhalePacket>()
    // - announcements: only keep latest per sender peerID
    private val latestAnnouncementByPeer = ConcurrentHashMap<String, Pair<String, BluewhalePacket>>()
    ...
    fun start() {
        ...
        cleanupJob = scope.launch(Dispatchers.IO) {
            while (isActive) {
                try {
                    delay(com.bluewhale.android.util.AppConstants.Sync.CLEANUP_INTERVAL_MS)
    ...
    fun onPublicPacketSeen(packet: BluewhalePacket) {
        ...
        if (isBroadcastMessage) {
            ...
            if (age > com.bluewhale.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
                Log.d(TAG, "Ignoring stale ANNOUNCE (age=${age}ms > ${com.bluewhale.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
                return
            }
    ...
    private fun sendRequestSync() {
        ...
        val packet = BluewhalePacket(
            ...
            payload = payload,
            ttl = com.bluewhale.android.util.AppConstants.SYNC_TTL_HOPS // neighbors only
        )
        // Sign and broadcast
    ...
    private fun sendRequestSyncToPeer(peerID: String) {
        ...
        val packet = BluewhalePacket(
            ...
            timestamp = System.currentTimeMillis().toULong(),
            payload = payload,
            ttl = com.bluewhale.android.util.AppConstants.SYNC_TTL_HOPS // neighbor only
        )
    ...
    fun handleRequestSync(fromPeerID: String, request: RequestSyncPacket) {
        ...
        for ((_, pair) in latestAnnouncementByPeer.entries) {
            ...
            if (!mightContain(idBytes)) {
                ...
                val toSend = pkt.copy(ttl = com.bluewhale.android.util.AppConstants.SYNC_TTL_HOPS)
    ...
    private fun buildGcsPayload(): ByteArray {
        ...
        val list = ArrayList<BluewhalePacket>()
        // announcements
